...
class SudokuGame {
    ...
    async renderAdventureGrid() {
        ...
        // Auth Check & Fetch
        if (db) {
            // Wait for Auth if needed? For now assuming auth listener initialized, but we check currentUser
            if (auth && auth.currentUser) {
                try {
                    // Fetch all scores for this user details to find passed adventure levels
                    // Since "startsWith" is tricky, we fetch all adventure scores for user
                    // Using a range query on challengeId for efficiency if possible, or just client filtering
                    // Range for 'ADV-LVL-': >= 'ADV-LVL-' and <= 'ADV-LVL-\uf8ff'
                    const snapshot = await db.collection("scores")
                        .where("uid", "==", auth.currentUser.uid)
                    ...
                    console.log("Adventure Progress Loaded:", completedSet);
                } catch (e) {
                    console.error("Error fetching adventure progress:", e);
                }
            } else {
                console.log("Adventure: No user logged in, cannot fetch progress.");
            }
        }
            ...
            btn.onclick = () => {
                if (isCompleted) {
                    ...
                    Swal.fire({
                        ...
                        text: 'Ya has superado este nivel.',
                        icon: 'success',
